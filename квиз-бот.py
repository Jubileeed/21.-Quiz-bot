...
def start(message):
    ...
    start_button = types.InlineKeyboardButton('Начать', callback_data='start_quiz')
    markup.add(start_button)

    # Отправляем сообщение с инлайн-кнопкой "Начать"
    bot.send_message(message.chat.id, 'Привет! Я бот-викторина! Нажмите кнопку "Начать", чтобы начать викторину.', reply_markup=markup)
...
def ask_question(message):
    ...
    if current_question < len(quiz_data):
        ...
        # Викторина закончена
        bot.send_message(message.chat.id, f"Викторина окончена! Ваши баллы: {points}.")
        current_question = 0  # Сбрасываем для возможности начать заново

# Функция для проверки ответа
@bot.callback_query_handler(func=lambda call: call.data in [opt for q in quiz_data for opt in q["options"]])
def check_answer(call):
    ...
    if call.data == correct_answer:
        ...
        bot.answer_callback_query(call.id, text=incorrect_response + f" Правильный ответ: {correct_answer}.")
